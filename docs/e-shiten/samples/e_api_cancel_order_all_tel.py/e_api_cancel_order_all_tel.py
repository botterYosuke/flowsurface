...
# p7/46 No.7-2 CLMKabuCancelOrderAll を参照してください。
#
# 7-2 CLMKabuCancelOrderAll
#  1	sCLMID	メッセージＩＤ	char*	I/O	"CLMKabuCancelOrderAll"
...
#  4	sSecondPassword	第二パスワード	char[48]	I	第二暗証番号（APIは省略不可）。''：第二暗証番号省略時。関連資料：「立花証券・ｅ支店・ＡＰＩ、インターフェース概要」の「３－２．ログイン、ログアウト」参照
...
# 備考:
#       
def func_cancel_order_all(int_p_no, class_login_property):
 ...
 # コマンド
 str_key = '"sCLMID"'
 str_value = 'CLMKabuCancelOrderAll' # 注文訂正。
 req_item.append(class_req())
 req_item[-1].add_data(str_key, str_value)
...
if __name__ == "__main__":
 ...
 print()
 print('-- 注文一括取消 -------------------------------------------------------------')
 dic_return = func_cancel_order_all(my_login_property.p_no, my_login_property)
 ...
 print('メッセージＩＤ:\t', dic_return.get('sCLMID'))
