...
**設計判断**:
- 初期値は `1`（`fetch_add` で先に現在値を返して +1 するため、最初の p_no は "1"）。
- `Ordering::Relaxed` で十分。p_no はユニーク性だけが必要で、他の変数との順序保証は不要。
- 公式サンプルはファイル永続化しているが、現段階ではプロセス内カウンタのみ。セッション永続化（4.1）実装時に `REQUEST_COUNTER` の初期値をファイルから復元する拡張が想定される。
- `pub fn` にしてあるので、将来的に外部から初期値をセットする API を追加しやすい。

**テスト**: 4件追加。`next_p_no` 連続呼び出しのインクリメント検証、各リクエスト型で p_no が "1" ハードコードでないこと・連続生成で異なることを検証。
...
セッション永続化時に `REQUEST_COUNTER` の初期値をファイルから復元する拡張は未実装（4.1 と同時に対応予定）。
...
### 5.2 テスト品質
- 38テスト（ユニット + mockito 統合テスト）で主要パスをカバー（元21 + 修正で+13追加）
- 境界ケース（`"*"` フィールド、未読書面フラグ、認証エラー）もテスト済み
- 新規追加分: HTTP メソッド検証、p_no 動的生成検証、パスワードエンコード検証、API エラーハンドリング検証
...
**テスト結果**: 45テスト全パス（元25 → 38 tachibana + 7 その他）
...
- `REQUEST_COUNTER` (`AtomicU64`) のセッション永続化: `store()` / `load()` で初期値を復元する拡張が必要。`Ordering::Relaxed` のままで OK。
...
**2026-04-09 更新**: セクション2の重大な差異4件はすべて修正済み。TDD（テスト駆動開発）アプローチで実装し、各修正に対して先にテストを書いて失敗を確認してから実装した。テスト数は 25 → 45 に増加（tachibana 関連: 25 → 38）。

**現状**: 本番接続テストに進める状態。残る作業は中優先度の UX 改善（セッション永続化・自動再ログイン）と低優先度の EVENT I/F 実装。
...
| シンボル | 種類 | 用途 |
|---------|------|------|
| `REQUEST_COUNTER` | `static AtomicU64` | グローバルリクエスト通番カウンタ |
| `next_p_no()` | `pub fn` | 次の通番を文字列で返す |
