...
### Phase 3: D1 自動再生（Tick ハンドラ内スロットリング）✅
...
| `src/main.rs` | Tick ハンドラで `is_all_d1_klines()` による分岐を追加、`process_d1_tick()` 呼び出し、既存 `end_time` 判定を else 側へ移設 | ✅ Phase 3 |
| `src/replay.rs` | `ReplayState` に `d1_virtual_elapsed_ms: f64` を追加、`advance_d1()` 自由関数と `process_d1_tick()` ヘルパー、`D1TickResult` 構造体を実装 | ✅ Phase 3 |
| `src/main.rs` | Play ハンドラで Phase 2 の `resume_status = Paused` ブロックを完全削除 | ✅ Phase 3 |
...
### Phase 3 実装（2026-04-12）

**変更ファイル**: `src/replay.rs`, `src/main.rs`

TDD（Red-Green-Refactor）で実装。各サイクルで RED 確認 → 最小実装 → GREEN 確認を遵守。

**追加した API**:
- `replay::advance_d1(virtual_elapsed_ms, speed, current_time, elapsed_ms, next_kline_time) -> (jumped, reached_end)`: 仮想時間累積方式のスロットリング自由関数
- `replay::process_d1_tick(pb, virtual_elapsed_ms, elapsed_ms, next_kline_time) -> D1TickResult`: 1 Tick 分の D1 経路処理（advance_d1 + drain 判定 + Paused 遷移）
- `replay::D1TickResult { current_time, trades_collected }`: process_d1_tick の戻り値
- `ReplayState::d1_virtual_elapsed_ms: f64`: 仮想時間累積カウンタ（`last_tick` と同階層）

**主要変更**:
- Tick ハンドラに `is_all_d1_klines()` による分岐を追加。D1-only 時は `process_d1_tick()` 経由で次の kline へ離散ジャンプ
- Play ハンドラから Phase 2 の「D1 → `resume_status = Paused`」ブロックを完全削除（D1 でも自動再生で Playing 開始）
- 既存 `pb.current_time >= pb.end_time` の自動停止判定は非 D1 経路の else 側にのみ残す（D1 経路では `advance_d1` の `reached_end` で判定）

**設計判断**:
- **`process_d1_tick` ヘルパー抽出**: Tick ハンドラ本体は Iced の `Task` まわりが絡んでユニットテストしづらいため、D1 経路のロジックを純粋関数 `process_d1_tick` に抽出して unit-testable にした。Tick ハンドラ側はこの関数を呼ぶだけの薄い統合層
- **`advance_d1` を自由関数化**: `&mut` 引数を取る自由関数にすることで、`PlaybackState` 全体を組み立てずにローカル変数だけでテスト可能にした
- **trade_buffers ドレインスキップ条件**: `is_all_d1_klines()` は取引所非依存（Binance D1 + heatmap 混在ケースが成立する）のため、「D1 経路だから常にスキップ」ではなく `trade_buffers.values().all(|b| b.trades.is_empty())` を満たす場合のみスキップする実装にした。Tachibana は trades が元々空なので意図通り skip され、Binance D1 + heatmap 混在時は drain される
- **浮動小数スラック `+ 1e-6`**: 9 回 × 100ms 累積で 999.9999 になるケースで 11 回目にずれるのを防ぐため、閾値判定に微小スラックを入れた
- **`d1_virtual_elapsed_ms` を `ReplayState` 直下に置く**: `last_tick` と同じく Tick ライフサイクルに紐づく状態のため。`PlaybackState` に追加すると永続化テストフィクスチャ群が広範囲に書き換えになる
- **手動 Step ハンドラでのカウンタリセットは実装しない**: Playing 中に Step を押した場合でも、次の Tick で最大 1 ジャンプ分早くなるだけで視覚的に問題ない。リセット忘れによるバグリスクを避けてシンプルさを優先
- **速度スライダの UI 変更なし**: `SPEEDS = [1.0, 2.0, 5.0, 10.0]` をそのまま「bars/sec」として再解釈するだけで、ラベル表示は変更不要

**追加テスト**（8 件、すべてグリーン）:

| # | テスト名 | 検証内容 |
|---|---------|---------|
| 1 | `advance_d1_jumps_when_threshold_reached` | speed=1.0・elapsed=1000ms 一発でジャンプ発火し current_time が更新される |
| 2 | `advance_d1_signals_end_when_no_next_kline` | next_kline=None で閾値到達時、reached_end=true、current_time は変化しない |
| 3 | `advance_d1_throttles_until_accumulated_threshold` | speed=1.0・elapsed=100ms を 9 回ではジャンプせず、10 回目で発火 |
| 4 | `advance_d1_speed_scales_jump_rate` | speed=10.0・elapsed=100ms で即ジャンプ（10x = 100ms/本） |
| 5 | `replay_state_default_d1_virtual_elapsed_is_zero` | `ReplayState::default()` で仮想カウンタが 0.0 |
| 6 | `process_d1_tick_skips_drain_when_all_buffers_empty` | trade_buffers が全空（Tachibana ケース）で drain 実行されず `trades_collected = None` |
| 7 | `process_d1_tick_drains_when_buffers_have_trades` | Binance D1 + heatmap 混在で trade_buffers に trades がある場合は `drain_until` が実行される（回帰テスト） |
| 8 | `process_d1_tick_pauses_on_reached_end` | 終端到達で `pb.status = Paused` に遷移し current_time は変化しない |

**TDD 違反検出と修正**:
- Cycle 2（終端検知）と Cycle 6c（Pause 遷移）で初回テストが「即パス」した（前のサイクルで意図せず実装済みだったため）。Iron law に従い、最小実装に巻き戻して RED を確認したうえで GREEN へ進めた

**最終結果**: 全 121 テストパス、ビルドクリーン、Phase 3 由来の clippy 警告なし
