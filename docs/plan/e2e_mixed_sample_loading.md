...
**スクリプト**: `tests/e2e_scripts/s30_mixed_sample_loading.sh`
...
### S31: 終端 ▶ 再スタートテスト
**スクリプト**: `tests/e2e_scripts/s31_replay_end_restart.sh`
...
bash tests/e2e_scripts/s30_mixed_sample_loading.sh

# S31 (通常 release ビルド)
cargo build --release
bash tests/e2e_scripts/s31_replay_end_restart.sh
```
...
# S30
FLOWSURFACE_EXE=./target/debug/flowsurface.exe bash tests/e2e_scripts/s30_mixed_sample_loading.sh 2>/dev/null

# S31
FLOWSURFACE_EXE=./target/debug/flowsurface.exe bash tests/e2e_scripts/s31_replay_end_restart.sh 2>/dev/null
```
...
- ✅ 計画書作成
- ✅ S30 テストスクリプト実装 (`tests/e2e_scripts/s30_mixed_sample_loading.sh`)
- ✅ S31 テストスクリプト実装 (`tests/e2e_scripts/s31_replay_end_restart.sh`)
