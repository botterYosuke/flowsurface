...
| Q: advance GUI ガード削除 + rewind reset 不変条件 | ✅ | 2026-04-23 | 0a1f406 | **ADR-0001 §3 GUI advance ガード撤廃**: [src/app/api/mod.rs](../../src/app/api/mod.rs) の `AgentSessionCommand::Advance { .. }` 400 分岐を削除し、`handle_agent_advance` (MVP 実装) にディスパッチするよう変更。MVP は `stop_on` / `include_fills` を 501 で拒否する progressive rollout（本体の advance・fills 集計・narrative outcome 非同期更新は実装）。`aggregate_updated_narratives` は非同期更新の集計困難のため常時 0。full parity は shared helper 抽出サブフェーズで対応予定。**ADR-0001 §4 rewind reset 不変条件実装**: [src/app/handlers.rs](../../src/app/handlers.rs) の `AgentMessage::RewindToStart` で `virtual_engine.reset()` + `mark_session_reset()` を発火し SessionLifecycleEvent::Reset による `client_order_id` UNIQUE map 自動クリアを有効化。NarrativeStore（SQLite DB）は reset 対象外（永続ナラティブ履歴を保持、headless と一致）。chart の「新 session 扱い」再描画は `agent_rewind → seek_to → reset_charts_for_seek` で実装済。[src/replay/controller/api.rs](../../src/replay/controller/api.rs) に `range_end_ms` / `step_size_ms` helper 追加。`cargo test --lib` 516 PASS、lib 新規警告 0（既存 11 維持） |
