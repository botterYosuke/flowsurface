...
| O: StepClock 縮退（P と束ねる） | ✅ | 2026-04-23 | ea24979 + 72aae56 + 9fbdd62 | StepClock から speed/ClockStatus/pause/play/resume_from_waiting 削除。`now_ms` / `step_size_ms` / `full_range` / `seek` / `set_step_size` / `tick_until` / `reached_end` のみに縮退。dispatcher.rs は `dispatch_tick(target_ms: u64)` へ置換（wall-clock 依存なし）。[src/replay/clock.rs](../../src/replay/clock.rs) / [src/replay/dispatcher.rs](../../src/replay/dispatcher.rs)。`ReplayController::is_paused` / `is_playing` / `speed_label` / `cycle_speed` 全削除。関連テスト ~48 件削除（pause/resume/speed/is_paused 等に依存していた旧テスト、仕様反転により機能退行ではなく仕様通り）。 |
| P: UI ボタン配線繋ぎ替え | 🔄 | 2026-04-23 | ea24979 + 72aae56 + 9fbdd62 | **部分完了**。`AgentMessage::{Step, Advance, RewindToStart}` 新設（[src/replay/mod.rs](../../src/replay/mod.rs)）と `Message::Agent` 統合、`ReplayController::{agent_step, agent_advance, agent_rewind}` 実装（[src/replay/controller/agent.rs](../../src/replay/controller/agent.rs)）、`src/app/handlers.rs::handle_agent` 実装、`src/app/view.rs` の ▶/⏭/⏮ ボタンを `Message::Agent` に配線。UI_ADVANCE_CAP_MS は `ReplayController` の associated const として公開。**未実装（専用サブフェーズに分離予定）**: ADR-0001 §4 の Reset 不変条件の完全実装 — SessionLifecycleEvent::Reset 発火 / `client_order_id` UNIQUE map クリア / NarrativeState::Reset / UI チャート「新 session 扱い」再描画。現状 `RewindToStart` は `virtual_engine.reset()` のみ。Phase 4a narrative outcome 自動更新は handle_tick 旧パターンを handle_agent に移植済（[src/app/handlers.rs:403-432](../../src/app/handlers.rs#L403)）。`cargo test --lib` 516 PASS（-48 は O で削除した clock/dispatcher 旧テスト）、`cargo check --bin` 通過。**hotfix 差分**: `stream.ticker()` → `stream.ticker_info().ticker.to_string()` 修正（ea24979 bin ビルド破綻を 72aae56 と追加 hotfix で解消）、`fill_time_ms` → `EpochMs::new().saturating_to_i64()` 既存パターンへ統一、agent_step/advance 戻り値を `Option` 化して非 Active 時の silent success を排除、`play_with_range` の orphan docstring 削除 |
| Q: advance GUI ガード削除 + rewind reset 不変条件 | 🔄 | 2026-04-23 | 0a1f406 + (未コミット hotfix2) | **ADR-0001 §3 GUI advance ガード撤廃**: [src/app/api/mod.rs](../../src/app/api/mod.rs) の `AgentSessionCommand::Advance { .. }` 400 分岐を削除し、`handle_agent_advance` (MVP 実装) にディスパッチ。MVP は `stop_on` / `include_fills` を 501 で拒否する progressive rollout（本体の advance・fills 集計・narrative outcome 非同期更新は実装）。`aggregate_updated_narratives` は非同期更新の集計困難のため常時 0。full parity は shared helper 抽出サブフェーズで対応予定。**ADR-0001 §4 rewind reset 不変条件実装**: `AgentMessage::RewindToStart` で `virtual_engine.reset()` + `mark_session_reset()` を発火し SessionLifecycleEvent::Reset による `client_order_id` UNIQUE map 自動クリアを有効化。NarrativeStore（SQLite DB）は reset 対象外（永続ナラティブ履歴を保持、headless と一致）。**hotfix2 レビュー指摘対応**: (a) `/api/agent/session/:id/rewind-to-start` HTTP ルート新設（`AgentSessionCommand::RewindToStart` variant + parser + GUI / headless ハンドラ、routing テスト 5 件追加）、(b) advance の `final_portfolio` を advance **後**の価格で評価するよう修正（unrealized_pnl/total_equity が古い価格で計算される High バグ）、(c) advance の Loading 状態判定を `is_loading()` 先行に変更（Loading→404 誤判定を 503 へ修正）、(d) `ticks_advanced` の除算を `ceil` 化し末端 partial step を 1 bar 計上するよう修正。`cargo test --lib` 521 PASS（+5 は rewind routing テスト）、lib 新規警告 0（既存 11 維持）。**未残作業**: `Started` / `Terminated` の GUI 側発火（toggle Live→Replay / Replay→Live 時）。headless は対応済だが GUI toggle 側は未実装。専用サブフェーズで対応予定。|
