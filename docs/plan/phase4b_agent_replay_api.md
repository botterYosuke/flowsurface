...
- [x] ~~`advance` を GUI ビルドで叩くと 400 `{"error":"instant mode requires headless"}` を返す~~
  ADR-0001 統合改訂により撤回。GUI でも `POST /api/agent/session/default/advance` を受理する。
- [x] ~~既存 `/api/replay/*`（UI リモコン API）は無改修で動作し続ける（既存 E2E `tests/s*.sh` が全 PASS）~~
  ADR-0001 統合改訂により撤回。旧 replay 制御ルートは削除し、E2E は agent session API へ移行する。
...
| TC | シナリオ | 期待値 |
|:-:|---|---|
| TC1 | `POST /api/replay/toggle {start,end}` → agent `/step` を 1 回 → 直後に UI `GET /api/replay/state` と agent observation を取得 | 両者の `clock_ms` が厳密に一致。OHLCV 末尾バーの close が一致 |
| TC2 | `POST /api/replay/toggle {start,end}` → agent `/order` で `client_order_id=cli_1` 発注 → UI `GET /api/replay/orders` | UI のリストに発注済みが現れ、`order_id` が agent レスポンスと一致 |
| TC3 | `POST /api/replay/toggle {start,end}` → agent `/order` で発注 → UI `/api/replay/toggle`（Live 戻し）→ 直後に agent `/order` を叩き **404** を確認 → 再度 `/api/replay/toggle {start,end}` → agent `/order` で **同一** `client_order_id=cli_1` 発注 | toggle 直後は 404（`Terminated` でセッション終了）。2 回目の初期化後の発注は **新規 `order_id`** を返す（§3.3 `Started` イベントで map クリア検証） |
| TC4 | agent `/rewind-to-start` → agent `/order` で発注 | agent 側は受理（§3.3 `Reset` イベントで map クリア検証）|
...
| R2 | `advance` 中に UI から旧 `/api/replay/pause` が入った場合の挙動 | 旧 pause ルートは削除済みで 404。GUI advance は `agent_advance_next` による明示 tick 進行のみ |
