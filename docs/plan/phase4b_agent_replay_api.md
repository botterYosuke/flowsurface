...
JSON バイト一致ではなく **正規化タプル `(ticker.exchange, ticker.symbol, side, qty, order_type)` の構造的等価** で判定する。JSON key 順・float 表記（`0.1` vs `0.10`）・空白の差異で誤判定しないため。

- `qty` / `price` は `f64` のビット等価ではなく、**64 bit 精度で丸めた 10 進文字列表現の一致**で比較（`format!("{:.8}", qty)` 等、丸め桁数は実装時に確定）
- `order_type` は `{ market: {} }` と `{ limit: { price: X } }` の enum 一致
...
- name: UI remote API freeze check (ADR-0001)
  run: |
    ...
    # route() マクロ / match パターンの tuple リテラル形式のみを検知する。
    # 正規表現: 行頭 + から始まり、前後空白を許容し ("METHOD", "/api/replay/...")
    # の形に一致する行のみ。テストコード内の裸文字列（"POST /api/replay/..." 等）は
    # この tuple 形式を取らないため false positive にならない。
