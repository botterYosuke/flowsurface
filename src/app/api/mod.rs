...
impl Flowsurface {
    ...
    /// `POST /api/agent/session/:id/advance` (GUI mode) のハンドラ。
    ///
    /// 実装本体は headless と同じレスポンス契約へ寄せた `handle_agent_advance_impl`
    /// に集約する。
    pub(crate) fn handle_agent_advance(
        ...
        self.handle_agent_advance_impl(request, reply_tx)
    }
