...
# 使い方: bash tests/e2e_replay_api.sh
#
...
# 2 時間幅の固定日時（最近の過去）
START_TIME="2026-04-10 20:00"
END_TIME="2026-04-10 22:00"
...
# ── Test 2: POST /api/replay/toggle (Live→Replay) ──
echo "[Test 2] POST /api/replay/toggle (Live→Replay)"
...
echo "[Test 3] GET /api/replay/status (after toggle)"
RESP=$(curl -s "${BASE}/api/replay/status")
...
sleep 3  # wait for Loading→Playing transition
RESP=$(curl -s -X POST "${BASE}/api/replay/pause")
assert_contains "pause returns status" "$RESP" '"mode":"Replay"'

# ── Test 7: step-forward (current_time 増加) ──
echo "[Test 7] POST /api/replay/step-forward"
...
# ── Test 8: step-backward (current_time 減少) ──
echo "[Test 8] POST /api/replay/step-backward"
BEFORE_TIME=$AFTER_TIME
RESP=$(curl -s -X POST "${BASE}/api/replay/step-backward")
sleep 2  # step-backward does chart rebuild
...
# ── Test 9: resume ──
echo "[Test 9] POST /api/replay/resume"
RESP=$(curl -s -X POST "${BASE}/api/replay/resume")
assert_contains "resume returns Playing" "$RESP" '"status":"Playing"'

# ── Test 10: toggle back to Live ──
echo "[Test 10] POST /api/replay/toggle (Replay→Live)"
RESP=$(curl -s -X POST "${BASE}/api/replay/toggle")
assert_contains "mode back to Live" "$RESP" '"mode":"Live"'

# ── Test 11: status after returning to Live ──
echo "[Test 11] GET /api/replay/status (back to Live)"
RESP=$(curl -s "${BASE}/api/replay/status")
assert_contains "mode is Live again" "$RESP" '"mode":"Live"'
# playback fields should be absent after returning to Live
assert_contains "no current_time in Live" "$RESP" '"mode":"Live"'

# ── Test 12: 不正パス → 404 ──
echo "[Test 12] GET /api/replay/unknown → 404"
STATUS=$(curl -s -o /dev/null -w "%{http_code}" "${BASE}/api/replay/unknown")
assert_http_status "unknown path returns 404" "$STATUS" "404"

# ── Test 13: 不正 JSON body → 400 ──
echo "[Test 13] POST /api/replay/play with bad JSON → 400"
# First toggle to Replay mode
curl -s -X POST "${BASE}/api/replay/toggle" > /dev/null
...
# ── Test 14: POST /api/replay/play with missing fields → 400 ──
echo "[Test 14] POST /api/replay/play with missing 'end' → 400"
...
# ── Test 15: POST /api/replay/play with empty body → 400 ──
echo "[Test 15] POST /api/replay/play with empty body → 400"
STATUS=$(curl -s -o /dev/null -w "%{http_code}" -X POST "${BASE}/api/replay/play" -d '')
assert_http_status "empty body returns 400" "$STATUS" "400"

# ── Test 16: double toggle round-trip ──
echo "[Test 16] Double toggle (Replay→Live→Replay→Live)"
# Currently in Replay from Test 13
RESP=$(curl -s -X POST "${BASE}/api/replay/toggle")  # Replay→Live
assert_contains "toggle to Live" "$RESP" '"mode":"Live"'
RESP=$(curl -s -X POST "${BASE}/api/replay/toggle")  # Live→Replay
assert_contains "toggle to Replay" "$RESP" '"mode":"Replay"'
RESP=$(curl -s -X POST "${BASE}/api/replay/toggle")  # Replay→Live
assert_contains "toggle back to Live" "$RESP" '"mode":"Live"'
