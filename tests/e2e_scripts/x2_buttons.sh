...
# --- TC-X2-07: CycleSpeed は current_time を変更しない（速度ラベルのみ変更） ---
# 旧仕様「CycleSpeed は pause + seek(range.start) を伴う」は廃止。
# 新仕様: CycleSpeed は速度のみ変更し、current_time に副作用なし（s27_cyclespeed_reset.sh で確認済み）。
set +e; wait_paused 5; set -e
curl -s -X POST "$API/replay/step-forward" > /dev/null
sleep 0.3
curl -s -X POST "$API/replay/step-forward" > /dev/null
sleep 0.3
PRE_CT=$(jqn "$(curl -s "$API/replay/status")" "d.current_time")
START_T=$(jqn "$(curl -s "$API/replay/status")" "d.start_time")
IS_AHEAD=$(bigt_gt "$PRE_CT" "$START_T")
[ "$IS_AHEAD" = "true" ] || fail "TC-X2-07-pre" "pre-condition: not ahead of start (pre=$PRE_CT start=$START_T)"
curl -s -X POST "$API/replay/speed" > /dev/null
POST_CT=$(jqn "$(curl -s "$API/replay/status")" "d.current_time")
EQ_CT=$(bigt_eq "$POST_CT" "$PRE_CT")
[ "$EQ_CT" = "true" ] \
  && pass "TC-X2-07: CycleSpeed 後も current_time は不変 (pre=$PRE_CT post=$POST_CT)" \
