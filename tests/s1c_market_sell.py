#!/usr/bin/env python3
"""s1c_market_sell.py — TOYOTA (7203) 成行売り E2E テスト（シナリオ 1-3）

検証シナリオ:
  Step 1: Live モードで起動・Tachibana セッション確認
  Step 2: 成行売り注文を送信（side=1, price=0）
  Step 3: 保有なしの場合はエラー（残高不足等）が返ることを確認
  Step 4: HTTP レスポンスが 200 であることを確認

注意: 実際に TOYOTA 株を保有している場合は約定するため要注意
前提: DEV_USER_ID / DEV_PASSWORD / DEV_SECOND_PASSWORD 環境変数が設定済みであること

使い方:
    DEV_USER_ID=... DEV_PASSWORD=... DEV_SECOND_PASSWORD=... python tests/s1c_market_sell.py
    pytest tests/s1c_market_sell.py -v
"""
...
def run_s1c() -> None:
    ...
    # Step 3: 成行売り注文（side=1, price=0）
    print()
    print("── Step 3: TOYOTA (7203) 100株 成行売り")
    ...
    if order_num:
        pass_(f"Step 3: 成行売り注文受付済み (order_number={order_num})")
    elif error:
        print(f"  INFO: エラー応答 (error={error}) — 保有株なしまたは市場時間外の可能性あり")
        pass_(f"Step 3: 成行売り API 疎通確認（エラー応答あり: {error}）")
    else:
    ...
    http_code = api_get_code("/api/tachibana/order")
    # POST の HTTP コード確認は別途 POST で行う（api_get_code は GET 用なので直接 POST）
    try:
...
def test_s1c_market_sell() -> None:
    ...
    if not os.environ.get("DEV_USER_ID") or not os.environ.get("DEV_PASSWORD"):
        pend("S1c", "DEV_USER_ID / DEV_PASSWORD が未設定 — スキップ")
        return
...
def main() -> None:
    ...
    print("=== TOYOTA (7203) 成行売り E2E テスト ===")
    backup_state()
