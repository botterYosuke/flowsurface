...
# 'sCLMID:CLMMfdsGetNewsBodyの利用方法
#
...
##	【要求】																				
##	No	項目			設定値													
##	1	sCLMID			CLMMfdsGetNewsBody													
##	2	p_ID			ニュースID（レコード毎にユニーク）。													
##					
##	【応答】				
##	No	項目			設定値													
##	1	sCLMID			CLMMfdsGetNewsBody													
##	2	p_ID			ニュースID（レコード毎にユニーク）。													
##	3	aCLMMfdsNewsBody	取得レコード情報（配列）。							※実際には最大１件しか設定されない。						
##		1	p_ID		ニュースID（レコード毎にユニーク）。													
##		2	p_DT		ニュニュースカテゴリリスト。							複数設定時は「,」区切り。						※１
##		5	p_GNL		ニュースジャンルリスト。							複数設定時は「,」区切り。						※１
##		6	p_ISL		ニュース関連銘柄コードリスト。							複数設定時は「,」区切り。						
##		7	p_HDL		ニュースヘッドライン（タイトル）。							※２						
##		8	p_TX		ニュースボディー（本文）。							※２						
...
# 引数1: p_no
# 引数2: str_p_ID 取得したいニュースIDを１つ指定。ニュースヘッダー取得結果から持ってくる。
# 引数3: 口座属性クラス
# 備考: 
def funcGetNewsHeader(int_p_no,
 ...
 # API request区分
 str_key = '"sCLMID"'
 str_value = 'CLMMfdsGetNewsBody' # ニュース本文取得を指示。
 req_item.append(class_req())
 req_item[-1].add_data(str_key, str_value)
...
def func_write_news_body_title(str_fname_output):
 try:
 ...
 str_text_out = str_text_out + 'p_TX' + '\n'
 ...
 str_text_out = str_text_out + 'ニュースボディー（本文）' + '\n'
...
# 機能: 取得したニュースデータを追記モードでファイルに書き込む
# 引数1: 出力ファイル名
# 引数2: 取得したニュースデータ（リスト型）
# 備考:
#   指定ファイルを開き、1〜2行目に取得する情報名を書き込み、3行目以降で取得した情報を書き込む。
#   引数2で渡されるリスト型ニュースデータは、1件のデータが入っているだけ。
def func_write_news_body_data(str_fname_output, list_return):
 try:
 ...
 str_p_TX = func_decode_base64_data(list_return[i].get('p_TX'))
 ...
 str_text_out = str_text_out + '"' + str_p_TX + '"' +'\n'


 fout.write(str_text_out)     # 処理済みのニュースデータをファイルに書き込む
 
 print('取得件数 len(list_return):', len(list_return))
 
 # ニュースヘッダーを取得できない場合。
 else :
 str_text_out = 'ニュースデータを取得できません。\n'
 print(str_text_out)
...
if __name__ == "__main__":
 ...
 # --- 利用時に変数を設定してください -------------------------------------------------------
 # コマンド用パラメーター -------------------    
 my_p_ID = '' # 本文を取得したいニュースID（レコード毎にユニーク）。ニュースヘッダーで取得したp_IDを指定する。例:20230825180000_OGL9800


 my_fname_output = 'news_header_' + my_p_ID + '.csv' # 書き込むファイル名。カレントディレクトリに上書きモードでファイルが作成される。
 ...
 # 取得レコード情報部分をリスト型で抜き出す。実際には1件のみの取得。
 my_list_return = dic_return.get('aCLMMfdsNewsBody')


 if my_list_return is not None:
 # 出力ファイルにタイトル行を書き込む。
 func_write_news_body_title(my_fname_output)
 
 print('-----------------')
 print('p_HDL : ニュースヘッダー')
 str_p_HDL = func_decode_base64_data(my_list_return[0].get('p_HDL'))
 print(str_p_HDL)


 print('-----------------')
 print('p_TX : ニュース本文')
 str_p_TX = func_decode_base64_data(my_list_return[0].get('p_TX'))
 print(str_p_TX)


 # 取得したニュースデータを追記モードでファイルに書き込む。
 func_write_news_body_data(my_fname_output, my_list_return)
 else:
 print('ニュース本文を取得できませんでした。')
 print('ニュースIDを確認してください。')
