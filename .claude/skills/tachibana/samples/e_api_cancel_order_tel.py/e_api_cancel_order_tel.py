...
# p7/46 No.7-1 CLMKabuCancelOrder を参照してください。
#
# 7-1 CLMKabuCancelOrder
#  1	sCLMID	メッセージＩＤ	char*	I/O	"CLMKabuCancelOrder"
...
#  6	sSecondPassword	第二パスワード	char[48]	I	第二暗証番号（APIは省略不可） ''：第二暗証番号省略時。関連資料：「立花証券・ｅ支店・ＡＰＩ、インターフェース概要」の「３－２．ログイン、ログアウト」参照
#  8	sOrderUkewatasiKingaku	注文受渡金額	char[16]	O	注文受渡金額
#  9	sOrderDate	注文日時	char[14]	O	注文日時（YYYYMMDDHHMMSS）
...
# 備考:
#       
def func_cancel_order(int_p_no,
 str_sOrderNumber,
 str_sEigyouDay,
 ...
 # コマンド
 str_key = '"sCLMID"'
 str_value = 'CLMKabuCancelOrder' # 注文訂正。
 req_item.append(class_req())
 req_item[-1].add_data(str_key, str_value)
...
if __name__ == "__main__":
 ...
 print()
 print('-- 注文取消 -------------------------------------------------------------')
 dic_return = func_cancel_order(my_login_property.p_no,
 my_sOrderNumber,
 my_sEigyouDay,
 my_login_property)
